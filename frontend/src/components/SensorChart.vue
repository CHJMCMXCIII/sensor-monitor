...
<script>
import { chartMargin, chartWidth, chartHeight } from '../config'
...
export default {
    ...
    setup(props) {
        ...
        const setAreaAndScale = (key) => {
            ...
            line = d3.line().x(d => xScale(d.time)).y(d => yScale(d[key]))
                .curve(d3.curveMonotoneX)
        ...
        const initDraw = (data, key) => {
            ...
            yScale.domain([mn - chartMargin.value, mx + chartMargin.value])
        ...
        // draw에서는 데이터 처리가 아닌 data를 통해서 차트를 그리는 것에 집중해야 함.
        const draw = (data, key) => {
            data.forEach(d => d.time = new Date(d.time))
